...
func (s *BrowseService) GetCommit(ctx context.Context, owner, repo, hash string) (*CommitInfo, error) {
	...
	verified, signer, _ := s.verifyCommitSignature(ctx, commit)
...
func (s *BrowseService) verifyCommitSignature(ctx context.Context, commit *object.CommitObj) (bool, string, error) {
	...
	key, err := s.repoSvc.db.GetSSHKeyByFingerprint(ctx, fp)
	if err != nil {
	...
	user, err := s.repoSvc.db.GetUserByID(ctx, key.UserID)
	if err != nil {
		return true, "", nil
	}
